package com.yz.rpc.transport.api.support.netty;
...
import com.yz.rpc.transport.api.support.AbstractClient;
import com.yz.rpc.transport.api.support.RPCTaskRunner;
        ...
        public void run() {
            if(!closedByServer){
                ...
                try{
                    ...
                    //出现异常,反复重连
                    log.error("重连失败,{}秒后重试...", ArcConstant.IDLE_INTERVAL);
                    //调度执行重连方法
                    futureChannel.eventLoop().schedule(retryExecutor, ArcConstant.IDLE_INTERVAL, TimeUnit.SECONDS);
                }
            }else{
                //服务器主动关闭连接
